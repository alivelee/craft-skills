...
"""
package_bundle.py
Assembles and validates a complete Xiaohongshu release bundle (Images, Text, Meta, Checklist).
"""
...
PROHIBITED_WORDS = ["最权威", "天下第一", "100%", "完全根治", "包教包会", "绝对有效", "国家级"]
...
def main():
    parser = argparse.ArgumentParser(description="Package Xiaohongshu Release Bundle")
    parser.add_argument("--dir", required=True, help="Target output bundle directory")
    parser.add_argument("--title", required=True, help="Note title (<= 20 chars)")
    ...
    parser.add_argument("--tags", nargs="*", default=["网球", "网球进阶", "科学运动"], help="Tags list")
    ...
    if violations:
        print(f"⚠️ [警告] 发现敏感/绝对化违禁词: {violations}，建议替换！")
    ...
    tags_formatted = " ".join([f"#{t}" if not t.startswith("#") else t for t in args.tags])
    ...
    print(f"✅ 小红书发布包已打包完成: {args.dir}")
    print(f"   - 标题: {args.title} ({len(args.title)}字)")
    print(f"   - 图片数量: {len(copied_imgs)} 张")
    print(f"   - 正文文件: {post_txt_path}")
